...
void pushbit(boolean  bit) {
  if (skipbits)
    ...
    if(notpu_edbits == 0) {
      ...
      notpu_edbytes--;
      if(notpu_edbytes == 0) {
        fwrite(Pu_buf, sizeof(unsigned char), PPBUFSIZ, Pu_File);
        pu_edbufs++;
        notpu_edbytes = PPBUFSIZ;
      }
      notpu_edbits = 8;
...
void pushexit(void) {
  ...
  fwrite(Pu_buf, sizeof(unsigned char), PPBUFSIZ - notpu_edbytes, Pu_File);
}
...
boolean pullbit(void) {
  ...
  if(notpu_edbits == 0) {
    if(notpu_edbytes == 0) {
      fread(Pu_buf, sizeof(unsigned char), PPBUFSIZ, Pu_File);
      pu_edbufs++;
      notpu_edbytes = PPBUFSIZ;
    }
